#pragma once

namespace provider {

	using namespace System;
	using namespace System::ComponentModel;
	using namespace System::Collections;
	using namespace System::Windows::Forms;
	using namespace System::Data;
	using namespace System::Drawing;

	/// <summary>
	/// Summary for Calendar
	/// </summary>
	public ref class Calendar : public System::Windows::Forms::Form
	{
	public:
		Calendar(void)
		{
			InitializeComponent();
			//
			//TODO: Add the constructor code here
			//
		}

	protected:
		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		~Calendar()
		{
			if (components)
			{
				delete components;
			}
		}
	private: System::Windows::Forms::Label^ label1;
	private: System::Windows::Forms::TextBox^ serviceTitle;

	private: System::Windows::Forms::Button^ button1;
	protected:

	private:
		/// <summary>
		/// Required designer variable.
		/// </summary>
		System::ComponentModel::Container ^components;

#pragma region Windows Form Designer generated code
		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		void InitializeComponent(void)
		{
			this->label1 = (gcnew System::Windows::Forms::Label());
			this->serviceTitle = (gcnew System::Windows::Forms::TextBox());
			this->button1 = (gcnew System::Windows::Forms::Button());
			this->SuspendLayout();
			// 
			// label1
			// 
			this->label1->AutoSize = true;
			this->label1->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 15));
			this->label1->Location = System::Drawing::Point(70, 67);
			this->label1->Name = L"label1";
			this->label1->Size = System::Drawing::Size(146, 25);
			this->label1->TabIndex = 0;
			this->label1->Text = L"Calendar name";
			this->label1->Click += gcnew System::EventHandler(this, &Calendar::label1_Click);
			// 
			// serviceTitle
			// 
			this->serviceTitle->Location = System::Drawing::Point(43, 118);
			this->serviceTitle->Name = L"serviceTitle";
			this->serviceTitle->Size = System::Drawing::Size(211, 20);
			this->serviceTitle->TabIndex = 1;
			// 
			// button1
			// 
			this->button1->Location = System::Drawing::Point(100, 154);
			this->button1->Name = L"button1";
			this->button1->Size = System::Drawing::Size(97, 33);
			this->button1->TabIndex = 2;
			this->button1->Text = L"Create";
			this->button1->UseVisualStyleBackColor = true;
			// 
			// Calendar
			// 
			this->AutoScaleDimensions = System::Drawing::SizeF(6, 13);
			this->AutoScaleMode = System::Windows::Forms::AutoScaleMode::Font;
			this->ClientSize = System::Drawing::Size(284, 261);
			this->Controls->Add(this->button1);
			this->Controls->Add(this->serviceTitle);
			this->Controls->Add(this->label1);
			this->Name = L"Calendar";
			this->Text = L"Calendar";
			this->ResumeLayout(false);
			this->PerformLayout();

		}
#pragma endregion
	private: System::Void label1_Click(System::Object^ sender, System::EventArgs^ e) {
	}
	};
}
